...
export const NavigationProvider: FC<TRouteProps> = ({ children }) => {
    ...
    const permitPaths = (path: string) =>
        setAllowedPaths((prev) => (!prev.includes(path) ? [...prev, path] : [...prev]));

    const clearPermitPath = (path: string) =>
        setAllowedPaths((prev) => prev.filter((item) => item !== path));

    const value = useMemo(() => ({ allowedPaths, permitPaths, clearPermitPath }), [allowedPaths]);
